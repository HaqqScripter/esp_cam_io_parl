...
static int reset(camera_sensor_t *sensor) {
	...
	// values
	ret = write_reg(sensor->sccb_address, SYSTEM_CTROL0, 0x01);
	...
	ret = write_regs(sensor->sccb_address, sensor_default_regs); // re-initial
...
static int set_image_options(camera_sensor_t *sensor) {
	...
	if (write_reg(sensor->sccb_address, TIMING_TC_REG20, reg20 | reg21)) {
...
static int set_framesize(camera_sensor_t *sensor, camera_framesize_t framesize) {
	...
	if (framesize == FRAMESIZE_QVGA) {
		...
		ret = write_regs(sensor->sccb_address, sensor_framesize_QVGA);
#if CONFIG_NT99141_SUPPORT_XSKIP
		ESP_LOGD(TAG, "Set FRAMESIZE_QVGA: xskip mode");
		ret = write_regs(sensor->sccb_address, sensor_framesize_QVGA_xskip);
#elif CONFIG_NT99141_SUPPORT_CROP
		ESP_LOGD(TAG, "Set FRAMESIZE_QVGA: crop mode");
		ret = write_regs(sensor->sccb_address, sensor_framesize_QVGA_crop);
#endif
	} else if (framesize == FRAMESIZE_VGA) {
		ESP_LOGD(TAG, "Set FRAMESIZE_VGA");
		// ret = write_regs(sensor->sccb_address, sensor_framesize_VGA);
		ret = write_regs(
			sensor->sccb_address,
		...
		ESP_LOGD(TAG, "Set FRAMESIZE_QVGA: xskip mode");
		ret = write_regs(sensor->sccb_address, sensor_framesize_VGA_xskip);
#elif CONFIG_NT99141_SUPPORT_CROP
		ESP_LOGD(TAG, "Set FRAMESIZE_QVGA: crop mode");
		ret = write_regs(sensor->sccb_address, sensor_framesize_VGA_crop);
#endif
	} else if (framesize >= FRAMESIZE_HD) {
		ESP_LOGD(TAG, "Set FRAMESIZE_HD");
		ret = write_regs(sensor->sccb_address, sensor_framesize_HD);
	} else {
...
static int set_gain_ctrl(camera_sensor_t *sensor, int enable) {
	int ret = 0;
	ret = write_reg_bits(sensor->sccb_address, 0x32bb, 0x87, enable);
...
static int set_exposure_ctrl(camera_sensor_t *sensor, int enable) {
	...
	int data = 0;
	// ret = write_reg_bits(sensor->sccb_address, 0x32bb, 0x87, enable);
	data = read_reg(sensor->sccb_address, 0x3201);
	ESP_LOGD(TAG, "set_exposure_ctrl:enable");
	if (enable) {
		ESP_LOGD(TAG, "set_exposure_ctrl:enable");
		ret = write_reg(sensor->sccb_address, 0x3201, (1 << 5) | data);
	} else {
...
static int set_agc_gain(camera_sensor_t *sensor, int gain) {
	...
	case 0:
		...
		write_reg(sensor->sccb_address, 0X301D, 0X00);
		break;

	case 1:
		ESP_LOGD(TAG, "set_agc_gain: 2x");
		write_reg(sensor->sccb_address, 0X301D, 0X0F);
		break;

	case 2:
		ESP_LOGD(TAG, "set_agc_gain: 4x");
		write_reg(sensor->sccb_address, 0X301D, 0X2F);
		break;

	case 3:
		ESP_LOGD(TAG, "set_agc_gain: 6x");
		write_reg(sensor->sccb_address, 0X301D, 0X37);
		break;

	case 4:
		ESP_LOGD(TAG, "set_agc_gain: 8x");
		write_reg(sensor->sccb_address, 0X301D, 0X3F);
		break;
...
static int set_aec_value(camera_sensor_t *sensor, int value) {
	...
	ret =
		write_reg_bits(sensor->sccb_address, 0x3012, 0x00, (value >> 8) & 0xff);
	ret = write_reg_bits(sensor->sccb_address, 0x3013, 0x01, value & 0xff);
...
static int set_ae_level(camera_sensor_t *sensor, int level) {
	...
	for (int i = 0; i < 5; i++) {
		ret +=
			write_reg(sensor->sccb_address, sensor_ae_level[5 * level + i][0],
...
static int set_wb_mode(camera_sensor_t *sensor, int mode) {
	...
	ret = write_reg(sensor->sccb_address, 0x3201, (mode != 0));
	...
	switch (mode) {
	case 1: // Sunny
		ret = write_reg16(sensor->sccb_address, 0x3290, 0x01) ||
			  write_reg16(sensor->sccb_address, 0x3291, 0x38) ||
			  write_reg16(sensor->sccb_address, 0x3296, 0x01) ||
			  write_reg16(sensor->sccb_address, 0x3297, 0x68) ||
			  write_reg16(sensor->sccb_address, 0x3060, 0x01);
	...
	case 2: // Cloudy
		...
		ret = write_reg16(sensor->sccb_address, 0x3290, 0x01) ||
			  write_reg16(sensor->sccb_address, 0x3291, 0x51) ||
			  write_reg16(sensor->sccb_address, 0x3296, 0x01) ||
			  write_reg16(sensor->sccb_address, 0x3297, 0x00) ||
			  write_reg16(sensor->sccb_address, 0x3060, 0x01);
		break;

	case 3: // INCANDESCENCE]
		ret = write_reg16(sensor->sccb_address, 0x3290, 0x01) ||
			  write_reg16(sensor->sccb_address, 0x3291, 0x30) ||
			  write_reg16(sensor->sccb_address, 0x3296, 0x01) ||
			  write_reg16(sensor->sccb_address, 0x3297, 0xCB) ||
			  write_reg16(sensor->sccb_address, 0x3060, 0x01);
		break;

	case 4: // FLUORESCENT
		ret = write_reg16(sensor->sccb_address, 0x3290, 0x01) ||
			  write_reg16(sensor->sccb_address, 0x3291, 0x70) ||
			  write_reg16(sensor->sccb_address, 0x3296, 0x01) ||
			  write_reg16(sensor->sccb_address, 0x3297, 0xFF) ||
			  write_reg16(sensor->sccb_address, 0x3060, 0x01);
		break;
...
static int set_special_effect(camera_sensor_t *sensor, int effect) {
	...
	ret = write_reg(sensor->sccb_address, 0x32F1, regs[0]) ||
		  write_reg(sensor->sccb_address, 0x32F4, regs[1]) ||
		  write_reg(sensor->sccb_address, 0x32F5, regs[2]) ||
		  write_reg(sensor->sccb_address, 0x3060, regs[3]);
...
static int set_brightness(camera_sensor_t *sensor, int level) {
	...
	ret = write_reg(sensor->sccb_address, 0x32F2, value);
...
static int set_contrast(camera_sensor_t *sensor, int level) {
	...
	ret = write_reg(sensor->sccb_address, 0x32FC, value1);
	ret = write_reg(sensor->sccb_address, 0x32F2, value2);
	ret = write_reg(sensor->sccb_address, 0x3060, 0x01);
...
static int set_saturation(camera_sensor_t *sensor, int level) {
	...
	{
		ret = write_reg(sensor->sccb_address, 0x32F3, regs[0]);
...
static int set_sharpness(camera_sensor_t *sensor, int level) {
	...
	ret = write_reg_bits(sensor->sccb_address, 0x5308, 0x40,
...
static int set_gainceiling(camera_sensor_t *sensor,
						   camera_gainceiling_t level) {
...
int nt99141_detect(int sccb_address, camera_sensor_id_t *id) {
	if (NT99141_SCCB_ADDR == sccb_address) {
...
static int init_status(camera_sensor_t *sensor) {
	...
	sensor->status.sharpness = (read_reg(sensor->sccb_address, 0x3301));
	sensor->status.denoise = get_denoise(sensor);
	sensor->status.ae_level = 0;
	sensor->status.gainceiling =
		read_reg16(sensor->sccb_address, 0x32F0) & 0xFF;
	sensor->status.awb =
		check_reg_mask(sensor->sccb_address, ISP_CONTROL_01, 0x10);
	sensor->status.dcw = !check_reg_mask(sensor->sccb_address, 0x5183, 0x80);
	sensor->status.agc = !check_reg_mask(sensor->sccb_address, AEC_PK_MANUAL,
										 AEC_PK_MANUAL_AGC_MANUALEN);
	sensor->status.aec = !check_reg_mask(sensor->sccb_address, AEC_PK_MANUAL,
	...
	sensor->status.vflip = check_reg_mask(sensor->sccb_address, TIMING_TC_REG20,
	...
	sensor->status.quality =
		read_reg(sensor->sccb_address, COMPRESSION_CTRL07) & 0x3f;
	sensor->status.special_effect = 0;
	sensor->status.wb_mode = 0;
	sensor->status.awb_gain =
		check_reg_mask(sensor->sccb_address, 0x3000, 0x01);
	sensor->status.agc_gain = get_agc_gain(sensor);
	sensor->status.aec_value = get_aec_value(sensor);
	sensor->status.aec2 = check_reg_mask(sensor->sccb_address, 0x3000, 0x04);
	return 0;
}
